...
export function createRelayCodemodeToolSet(args: CreateRelayCodemodeToolArgs): ToolSet {
  ...
  const codemode = createExecuteTool({
    ...
    description: [
      ...
      "The `openapi_describe_operation` cache is **invocation-local**. If you split these three steps across separate `codemode` calls, `cloudflare_request` will fail with `executionPlannerNote: no cached OpenAPI operation`. Always compose a single async function that calls all three in sequence.",
      "",
