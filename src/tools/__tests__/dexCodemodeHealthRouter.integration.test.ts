...
test('live-style DEX health MEMHQ2375GK1 route — openapi_search + resolve ordering; blocks hostname as device_id; visible fallback when codemode "Done"', async () => {
  ...
  assert.equal(fleetLiveRequestedPaths.length, 0, "resolver must List inventory endpoints only — never live with hostname");
  ...
  const hostnameLive = await execTool(meta, "cloudflare_request", {
    method: "GET" as const,
    path: `/accounts/{account_id}/dex/devices/${DEVICE_NEEDLE}/fleet-status/live`,
  });
  ...
  const liveOk = await execTool(meta, "cloudflare_request", {
    method: "GET" as const,
    path: `/accounts/{account_id}/dex/devices/${goodUuid}/fleet-status/live`,
  });
  assert.equal((liveOk as { ok?: boolean }).ok, true);
