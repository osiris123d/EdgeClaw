/**
 * Production Coder sub-agent — delegated coding assistance (Think facet).
 *
 * Isolated DO + Session via `subAgent(CoderAgent, name)` / `rpcCollectChatTurn`.
 * Extends {@link BaseSubAgentThink} (plain `Think`) — not {@link MainAgent}.
 */
...
import { CODER_IMPLEMENTATION_PATH_POLICY_MARKDOWN } from "../codingLoop/codingLoopImplementationPaths";

export class CoderAgentThinkFacet extends BaseSubAgentThink {
